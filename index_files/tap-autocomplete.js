...
function TapAutocomplete(params) {
...
TapAutocomplete.prototype.refresh = function() {
	 if (this.editor != null && jQuery('.CodeMirror').length > 0 && this.servicemode!=null) {
		...
		CodeMirror.commands.autocomplete = function(cm) {
			CodeMirror.tapHint(cm, CodeMirror.adqlHint, {
				webServicePath: _this.web_service_path,
				tapResource: _this.tap_resource,
				servicemode: _this.servicemode.toLowerCase(),
				jsontree: _this.jsontree,
				autocompleteInfo: _this.autocomplete_info,
				autocompleteLoader: _this.autocomplete_loader
			});
		}
...
TapAutocomplete.prototype.setValue = function(value){ if (this.editor != null && jQuery('.CodeMirror').length > 0) {this.editor.setValue(value);}};

TapAutocomplete.prototype.getValue = function(){ if (this.editor != null && jQuery('.CodeMirror').length > 0) {return this.editor.getValue();}};

TapAutocomplete.prototype.updateData = function(matches){ if (this.editor != null && jQuery('.CodeMirror').length > 0) {
	
		if (this.autocomplete_loader){
			jQuery("#" + this.autocomplete_loader).show();
		}
		if (matches){ 
			if (matches.length > 0) {
				for (var i = 0; i < matches.length; i++) {
					var str = jQuery.trim(matches[i]);
					var arr = str.split(".");
					for (var y = 0; y < arr.length; y++) {
						this.editor.availableTags.push(arr[y]);
					}
